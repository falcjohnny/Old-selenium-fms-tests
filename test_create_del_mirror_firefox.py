...
class TestCreateDelMirrorFirefox(unittest.TestCase):
    def setUp(self):
    ...
    def test_create_del_mirror_firefox(self):
        driver = self.driver
	...
	driver.find_element_by_xpath("//div[@col='0']").click()
        # Create Mirror
        driver.find_element_by_xpath("//button[contains(@data-template-url,'views/manage/mirror-menu.tpl.html')]").click()
	...
	driver.find_element_by_xpath("//span[@aria-label='Select box activate']").click()
        ...
        for i in range(60):
            try:
                if driver.find_element_by_xpath("//strong[contains(.,'The mrror has been created.')]").is_displayed(): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
	driver.find_element_by_xpath("//div[@col='0']").click()
	...
	driver.find_element_by_xpath("//a[contains(.,'Delete Mirror')]").click()
        ...
        for i in range(60):
            try:
                if driver.find_element_by_xpath("//strong[contains(.,'The mirror for virtual has been deleted.')]").is_displayed(): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        driver.find_element_by_xpath("//div[@col='0']").click()
