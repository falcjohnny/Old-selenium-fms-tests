...
class TestCreateDelSafeCacheFirefox(unittest.TestCase):
    def setUp(self):
    ...
    def test_create_del_safecache_firefox(self):
        driver = self.driver
	...
	time.sleep(1)
	driver.find_element_by_name("size").clear()
        driver.find_element_by_name("size").send_keys("10")
        driver.find_element_by_xpath("//button[@type='submit']").click()
        for i in range(60):
            try:
                if driver.find_element_by_xpath("//strong[contains(.,'The SafeCache has been created.')]").is_displayed(): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
	driver.find_element_by_xpath("//div[@col='0']").click()
	...
	driver.find_element_by_xpath("//a[contains(.,'Delete SafeCache')]").click()
        ...
        for i in range(60):
            try:
                if driver.find_element_by_xpath("//strong[contains(.,'The SafeCache has been deleted.')]").is_displayed(): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        driver.find_element_by_xpath("//div[@col='0']").click()
